"""
Linear HyperMARL for IPPO with Shared Weights.

HyperMARL generates both actor and critic parameters using a linear hypernetwork.
"""
...
class LinearHyperNetwork(nn.Module):
    """
    Linear HyperNetwork for generating weights and biases of target networks.

    This network takes agent embeddings as input and generates parameters for
    a target network (either actor or critic). Each agent gets its own set of
    weights while sharing the parameters of the hypernetwork.
    """
    ...
    use_bias: bool = False  # Whether to use bias in the hypernetwork
...
class ActorCritic(nn.Module):
    """
    ActorCritic network using hypernetworks to generate agent-specific parameters.

    The architecture uses linear hypernetworks to generate weights for both the actor
    and critic networks from agent embeddings, allowing parameter sharing while maintaining
...
def make_train(config):
    ...
    assert config["env"]["ENV_KWARGS"].get(
        "one_hot_encode_agent_id"
    ), "one_hot_ids must be True"
        ...
        def _select_action(params, obs, _rng):
            ...
            if action_type is ActionType.DISCRETE:
                ...
                pi = distrax.MultivariateNormalDiag(
                    loc=actor_mean, scale_diag=jnp.exp(actor_logtstd)
                )
