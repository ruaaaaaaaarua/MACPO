...
def _save_best_validation_checkpoint(
    ...
    *,
    ...
    checkpoint = generation / "training_state.msgpack"
    save_jax_training_checkpoint(
        checkpoint,
    ...
    metadata = {
        ...
        "sha256": {"jax_checkpoint": _sha256(checkpoint)},
    }
...
class ActorCritic(nn.Module):
    ...
    def __call__(self, x, x_critic):
        ...
        if self.is_continuous:
            ...
            effective_log_std = clamp_log_std(
                log_std,
                minimum=self.log_std_min,
                maximum=self.log_std_max,
            )
            return (actor_logits, effective_log_std), jnp.squeeze(critic, axis=-1)
    ...
    def train(rng, wb_run=None):
        ...
        def _select_action_eval(params, obs, _rng):
            ...
            if is_continuous:
                ...
                pi = distrax.Categorical(logits=actor_output)
                action = pi.sample(seed=_rng) if config.get("eval_stochastic") else pi.mode()
        ...
        def _update_step(runner_state, unused):
            ...
            for t in range(config["NUM_STEPS"]):
                ...
                # Step environment
                if is_continuous and config.get("POLICY_MODE") != "squashed_gaussian":
        ...
        rng, _rng = jax.random.split(rng)
        runner_state = (
            train_state, env_state, obsv, global_obsv,
            restored_rng if restored_rng is not None else _rng, episode_stats,
        )
        ...
        capture_interval = config.get("CAPTURE_VIDEO_INTERVAL", None)
        next_capture_video_step = (
            (global_step // capture_interval + 1) * capture_interval
        ...
        best_validation_return = _existing_best_validation_return(best_root)
        ...
        for update in range(start_update, config["NUM_UPDATES"]):
            ...
            if (global_step >= next_eval_step) or record_final_episode:
                ...
                if config.get("FIXED_EVAL_OUTPUT"):
                    ...
                    append_evaluation_record(
                        config["FIXED_EVAL_OUTPUT"],
                        eval_data,
                        training_episode=global_step // config["NUM_STEPS"],
                    )
                    ...
                    eval_data = run_eval_jax(
                        ...
                        global_step=global_step, capture_video=capture_video,
