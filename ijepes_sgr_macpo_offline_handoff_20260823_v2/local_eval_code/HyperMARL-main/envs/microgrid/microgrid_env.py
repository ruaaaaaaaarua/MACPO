...
class MicrogridEnv:
    ...
    def __init__(self, config_overrides=None):
        ...
        # Renewable (PV+WT) look-ahead.  Off by default so that every existing
        # variant's config dict -- and therefore its checkpoint fingerprint --
        # stays bit-identical.  Only a variant that sets these keys in
        # env_overrides pays for them.
        self.renewable_forecast_enable = bool(
            self.cfg.get("renewable_forecast_enable", False)
        )
        self.renewable_forecast_horizons = tuple(
            sorted({int(h) for h in self.cfg.get(
                "renewable_forecast_horizons", [2, 4, 6]) if int(h) > 0})
        )
        # Relative std of the multiplicative generation forecast error at a
        # 1-hour lead; the applied error grows as sqrt(horizon).  0.0 gives a
        # perfect forecast (an upper bound, not a realistic setting).
        self.renewable_forecast_noise_std = float(
            self.cfg.get("renewable_forecast_noise_std", 0.15)
        )
        self.renewable_forecast_dim = (
            2 * len(self.renewable_forecast_horizons)
            if self.renewable_forecast_enable else 0
        )
        ...
        # Env-v6.1 可选无功动作维:再加 1 维,控制微网在 PCC 的逆变器无功注入。
        # 默认关闭,关闭时动作维与历史 checkpoint 逐位一致。
        self.pcc_q_action_enable = bool(self.cfg.get("pcc_q_action_enable", False))
        self.pcc_q_action_index = None
        if self.pcc_q_action_enable:
            self.pcc_q_action_index = self.action_dim
            self.action_dim += 1
        ...
        # 无功能力来自逆变器接口资产(PV + 电池):|Q| <= sqrt(S^2 - P_inv^2)。
        # 视在容量默认取 1.2 * (pv_cap + bat_power),可用 pcc_q_apparent_cap_kva
        # 覆盖(标量或逐智能体列表)。
        pcc_q_cap_cfg = self.cfg.get("pcc_q_apparent_cap_kva")
        if pcc_q_cap_cfg is None:
            self.pcc_q_apparent_cap_kva = (
                1.2 * (self.pv_cap + self.bat_power)
            ).astype(np.float32)
        else:
            self.pcc_q_apparent_cap_kva = np.broadcast_to(
                np.asarray(pcc_q_cap_cfg, dtype=np.float32), (self.agent_num,)
            ).astype(np.float32).copy()
        if self.pcc_q_action_enable and not np.all(
            np.isfinite(self.pcc_q_apparent_cap_kva)
            & (self.pcc_q_apparent_cap_kva >= 0.0)
        ):
            raise ValueError("pcc_q_apparent_cap_kva must be finite and non-negative")
    ...
    def step(self, actions):
        ...
        pcc_q_setpoint_actions = np.zeros(self.agent_num, dtype=np.float32)
        if self.pcc_q_action_enable:
            pcc_q_setpoint_actions = raw_actions[:, self.pcc_q_action_index].astype(
                np.float32
            )
        ...
        # Env-v6.1 无功注入:正=容性(降低无功净需求、支撑电压),负=感性。
        # 能力受当步逆变器有功占用约束:|Q| <= sqrt(S^2 - (pv + |p_bat|)^2)。
        pcc_q_inj_kvar = np.zeros(self.agent_num, dtype=np.float64)
        pcc_q_avail_kvar = np.zeros(self.agent_num, dtype=np.float64)
        if self.pcc_q_action_enable:
            inverter_p_kw = np.abs(p_bat).astype(np.float64) + pv_now.astype(np.float64)
            apparent_cap = self.pcc_q_apparent_cap_kva.astype(np.float64)
            pcc_q_avail_kvar = np.sqrt(
                np.maximum(apparent_cap * apparent_cap - inverter_p_kw * inverter_p_kw, 0.0)
            )
            pcc_q_inj_kvar = (
                pcc_q_setpoint_actions.astype(np.float64) * pcc_q_avail_kvar
            )
            pcc_q_kvar = pcc_q_kvar - pcc_q_inj_kvar
        ...
        info = {
            ...
            "pcc_q_inj_kvar": pcc_q_inj_kvar.astype(float).tolist(),
            "pcc_q_avail_kvar": pcc_q_avail_kvar.astype(float).tolist(),
    ...
    def _renewable_forecast_facts(self, agent_id, t):
        """Noisy look-ahead on local renewable surplus / deficit.

        For each horizon H the agent sees two clipped scalars over hours
        ``t+1 .. t+H``: the energy by which its own PV+WT is forecast to
        exceed its electric load, and the energy by which it falls short.
        The forecast error is multiplicative on generation only (load is far
        more predictable) and grows as ``noise_std * sqrt(H)``, so the 6-hour
        view is materially worse than the 2-hour one.  Like the heat-load
        facts this never reads past the end of the episode.
        """
        facts = []
        for horizon in self.renewable_forecast_horizons:
            stop = min(self.T, int(t) + int(horizon) + 1)
            gen = (self.profiles["pv"][agent_id, int(t) + 1:stop]
                   + self.profiles["wt"][agent_id, int(t) + 1:stop])
            load = self.profiles["load_e"][agent_id, int(t) + 1:stop]
            if self.renewable_forecast_noise_std > 0.0:
                err = 1.0 + self.renewable_forecast_noise_std * np.sqrt(
                    float(horizon)) * self._rng.randn()
                gen = gen * float(np.clip(err, 0.0, 2.0))
            net = (gen - load) * self.dt
            scale = max(
                float(self.pv_cap_safe[agent_id] + self.wt_cap_safe[agent_id])
                * self.dt * float(horizon),
                EPS,
            )
            facts.append(np.clip(float(np.sum(np.maximum(net, 0.0))) / scale,
                                 0.0, 1.0))
            facts.append(np.clip(float(np.sum(np.maximum(-net, 0.0))) / scale,
                                 0.0, 1.0))
        return facts
    ...
    def _get_obs(self):
        ...
        for agent_id in range(self.agent_num):
            ...
            if self.renewable_forecast_enable:
                obs_values.extend(self._renewable_forecast_facts(agent_id, t))
