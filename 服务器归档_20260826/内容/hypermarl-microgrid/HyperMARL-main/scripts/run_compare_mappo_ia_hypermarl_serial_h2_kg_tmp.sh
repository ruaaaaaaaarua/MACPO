#!/usr/bin/env bash
# Temporary serial compare: H2 prices in yuan/kg (h2_price_unit=kg).
#
# Budget: TOTAL_TIMESTEPS = 5000 * 24 = 120000 (not multiplied by NUM_ENVS)
# Parallel: NUM_ENVS=10, CPU threads=8
# Environment: FullCDA-ReserveDemand + Price30/kg (internal P2P via CDA)
#
# Usage (from repo root):
#   bash HyperMARL-main/scripts/run_compare_mappo_ia_hypermarl_serial_h2_kg_tmp.sh smoke
#   bash HyperMARL-main/scripts/run_compare_mappo_ia_hypermarl_serial_h2_kg_tmp.sh serial
...
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/result/compare_mappo_ia_hypermarl_h2kg_10env_5kep_${RUN_TS}}"
...
echo "${OUT_ROOT}" > "${REPO_ROOT}/result/.last_compare_mappo_ia_hypermarl_h2kg_out"

run_mappo_ia() {
  local run_tag="mappo_ia_h2kg_${SEED}_${RUN_TS}_$$"
  ...
  echo "[MAPPO-IA-CTDE h2/kg] log=${log} TOTAL_TIMESTEPS=${TOTAL_TIMESTEPS} NUM_ENVS=${NUM_ENVS}"
  (
    cd "${HM_ROOT}"
    python baselines/MAPPO/mappo_ff_shared_weights.py \
      --config-name=mappo_ff_independent_actors_microgrid \
      ALG="MAPPO-IA-CTDE-5kEp-h2kg" \
      EXP_NAME="mappo_ff_independent_actors_microgrid" \
      RUN_NAME="microgrid__MAPPO-IA-h2kg__seed${SEED}__${run_tag}" \
      SEED="${SEED}" \
...
run_hypermarl_mappo() {
  local run_tag="hypermarl_h2kg_${SEED}_${RUN_TS}_$$"
  export XLA_PYTHON_CLIENT_MEM_FRACTION="${XLA_PYTHON_CLIENT_MEM_FRACTION:-0.70}"
  ...
  echo "[HyperMARL-MAPPO h2/kg] log=${log} TOTAL_TIMESTEPS=${TOTAL_TIMESTEPS} NUM_ENVS=${NUM_ENVS}"
  (
    cd "${HM_ROOT}"
    python baselines/MAPPO/mappo_ff_shared_weights_mlp_hypernets_microgrid.py \
      ...
      ALG="HyperMARL-MAPPO-5kEp-h2kg" \
      EXP_NAME="mappo_ff_shared_weights_mlp_hypernets_microgrid" \
      RUN_NAME="microgrid__HyperMARL-MAPPO-h2kg__seed${SEED}__${run_tag}" \
      SEED="${SEED}" \
      TOTAL_TIMESTEPS="${TOTAL_TIMESTEPS}" \
      NUM_ENVS="${NUM_ENVS}" \
      WANDB_MODE="${WANDB_MODE}" \
...
export_actors() {
  ...
  if [[ -n "${ckpt_ia}" ]]; then
    python "${HM_ROOT}/scripts/export_actor_weights.py" \
      ...
      --algorithm "MAPPO-IA-CTDE-5kEp-h2kg" \
      --seed "${SEED}"
  else
  ...
  if [[ -n "${ckpt_hyper}" ]]; then
    python "${HM_ROOT}/scripts/export_actor_weights.py" \
      ...
      --algorithm "HyperMARL-MAPPO-5kEp-h2kg" \
      --seed "${SEED}"
  else
...
postprocess() {
  python "${HM_ROOT}/scripts/plot_compare_mappo_ia_hypermarl.py" \
    --out-root "${OUT_ROOT}" \
    --num-envs "${NUM_ENVS}" \
    --ia-tag "MAPPO-IA-CTDE-5kEp-h2kg" \
    --hyper-tag "HyperMARL-MAPPO-5kEp-h2kg" \
    --env-note "FullCDA-ReserveDemand + Price30/kg（h2_price_unit=kg）；内部 P2P 互济经 CDA 撮合"
  export_actors
...
case "${MODE}" in
  ...
  hypermarl)
    ...
    ;;
  smoke|serial)
    echo "=== serial MAPPO-IA-CTDE -> HyperMARL-MAPPO h2/kg (${MODE}) ==="
