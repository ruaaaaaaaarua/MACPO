...
当前 `action_dim = 5`。每个动作维度由策略输出为 `[-1, 1]`，环境再映射为物理量：
...
| a3 | `h2_bid_price` | H2 CDA 报价 | 映射到 `[lambda_h2_sell, lambda_h2_buy]` 或动态 gas-node H2 价格 |
| a4 | `P_ht` | 储氢罐充放功率 | `a4 * h2_tank_power`，正值充氢、负值放氢 |

订单数量不是动作。电力和 H2 的买/卖单数量由环境根据净需求自动生成，策略只控制报价和设备调度。
...
- `net_h2_demand > 0`：买 H2。
- `net_h2_demand < 0`：卖 H2。
- raw 数量为 `abs(net_h2_demand)`。
- 报价来自动作 `a3` 映射后的 `h2_bid_price`。
...
- `h2_cap_aware_buy_enable=True` 时，买单会按未来 headroom 减去 pending H2 后限幅。
- 如果 `t + h2_delivery_lag > T`，该买单会被 horizon clipping 到 0，避免 episode 结束后才到货。
...
| `delivered_h2_energy` | 当步实际入罐的 delayed H2 |
