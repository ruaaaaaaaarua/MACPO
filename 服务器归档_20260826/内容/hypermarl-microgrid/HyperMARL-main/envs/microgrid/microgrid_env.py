...
与原实现相比,这一版:
- 动作降为 5 维:[P_el, P_bat, elec_price, h2_price, P_ht],
  电力/氢能市场的 "报量比例" 不再由策略决定,订单量等于本步净需求绝对值。
- 热负荷固定 100% 由氢承担,不足部分由外部氢市场兜底,不再引入天然气分流动作。
- 电力/氢市场均每步即时 CDA 撮合,不使用协议时刻、运输延迟或 pending 氢交割。
- 每步奖励加入 SOC / H2 罐 的逐步惩罚项:
      penalty_t = α / (T - t) * Σ_i [(SOC_i - 0.5)^2 + (H2_ratio_i - 0.5)^2]。
- obs 16 维:在原 13 维基础上加入本地 SOC 偏差、H2 偏差和时间紧迫度。
...
class MicrogridEnv:
    ...
    def __init__(self):
        ...
        self.h2_price_unit = str(self.cfg.get("h2_price_unit", "kwh")).strip().lower()
        if self.h2_price_unit == "kg":
            lhv = float(self.cfg["LHV_H2"])
            self.lambda_h2_buy /= lhv
            self.lambda_h2_sell /= lhv
            self.h2_price_min /= lhv
            self.h2_price_max /= lhv
            self.h2_price_init /= lhv
        elif self.h2_price_unit not in ("kwh", ""):
            raise ValueError(f"Unsupported h2_price_unit: {self.h2_price_unit}")
    ...
    def reset(self):
        ...
        self.t = 0
        self.soc = np.full(self.agent_num, self.cfg["soc_min"], dtype=np.float32)
        self.h2_level = (
            self.h2_tank_cap * self.cfg["h2_tank_min_ratio"]
        ).astype(np.float32)
    ...
    def step(self, actions):
        ...
        for agent_id in range(self.agent_num):
            ...
            h2_bid_price[agent_id] = self._scale_price(
                action[3],
                h2_external_sell_prices_t[agent_id],
                h2_external_buy_prices_t[agent_id],
            )
        ...
        if (self.h2_internal_cda_enable or self.h2_p2p_enable) and is_agreement_step:
            for agent_id in range(self.agent_num):
                ...
                if (
                    net_h2_demand[agent_id] >= -EPS
                    and h2_order_quantity[agent_id] > EPS
                ):
                    h2_order_side[agent_id] = "buy"
                    if (
                        self.h2_cap_aware_buy_enable
                        and self.h2_market_lag_enable
                        and self.h2_delivery_lag > 0
                    ):
                        deliverable_within_episode = (
                            int(t) + int(self.h2_delivery_lag) <= int(self.T)
                        )
                ...
                elif net_h2_demand[agent_id] < -EPS and h2_order_quantity[agent_id] > EPS:
        ...
        for agent_id in range(self.agent_num):
            h2_sold = h2_result["sell_matched"].get(agent_id, 0.0)
            h2_bought = h2_result["buy_matched"].get(agent_id, 0.0)
            if self.h2_market_lag_enable and self.h2_delivery_lag > 0:
                h2_bought = 0.0
            e_h2_ext[agent_id] = (
                net_h2_demand[agent_id] + h2_sold - h2_bought
            )
        ...
        c_h2 = float(
            np.sum(h2_external_buy_prices_t * np.maximum(e_ext_np, 0.0))
        ...
        if (
            ...
            terminal_h2_shortfall_value_term = coef * shortfall_energy
        ...
        # Training reward uses only realized operating cost. Penalties and
        # market bonuses remain available in info for diagnostics, but they do
        # not shape the optimization objective.
