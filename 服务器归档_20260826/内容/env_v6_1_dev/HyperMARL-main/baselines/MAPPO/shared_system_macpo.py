...
class SharedSystemMACPOUpdater:
    ...
    def update(
        ...
        *,
        ...
        # ``irrecoverable`` is kept as a diagnostic only.  The E1 bigbatch run
        # (2026-07-27) tried the textbook CPO branch -- trusting the linearized
        # surrogate to trade reward against a recoverable cost gap -- and the
        # surrogate proved wildly optimistic: it claimed ~130-point cost drops
        # per accepted step while the sampled cost kept rising (final policy:
        # 1/28 safe days vs the 10/28 baseline).  So when the sampled policy is
        # infeasible we do not trust the surrogate's feasibility claim at all:
        # the whole trust region is spent on cost reduction.  With the
        # episode-sum surrogate scale the recovery line search is now a real
        # check rather than a vacuous one.
        irrecoverable = (
            cost_gap > self.tolerance
            and (
                cost_quadratic <= self.tolerance
                or cost_gap * cost_gap / cost_quadratic >= 2.0 * self.max_kl
            )
        )
        ...
        return accepted_params, {
            ...
            "kl_after": kl_after,
            "cost_gap": cost_gap,
            # Trust-region geometry, for post-hoc diagnosis of the update mix.
            "cost_recoverable": bool(not irrecoverable),
            "cost_quadratic": cost_quadratic,
            "reward_quadratic": reward_quadratic,
            "reward_cost_cross": reward_cost_cross,
            "max_linear_cost_drop": (
                (2.0 * self.max_kl * cost_quadratic) ** 0.5
                if cost_quadratic > 0.0
                else 0.0
            ),
        }
