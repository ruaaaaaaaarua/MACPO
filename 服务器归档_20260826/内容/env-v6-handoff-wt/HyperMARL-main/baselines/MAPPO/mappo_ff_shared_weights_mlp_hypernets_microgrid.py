"""
MLP Hypernetworks for MAPPO with shared weights on Microgrid.

Actor parameters are generated by an MLP hypernetwork; the centralized critic uses
a shared fixed MLP on concatenated global observations.
"""
...
class MLPHyperNetwork(nn.Module):
    """MLP HyperNetwork for generating actor network weights and biases."""
...
class ActorCritic(nn.Module):
    """
    Actor-Critic with actor-only MLP hypernetwork and shared fixed centralized critic.

    The actor uses hypernetwork-generated weights per agent; the critic is a standard
    MLP applied to global observations (x_critic).
    """
    action_dim: int
    num_agents: int
    ...
    hypernet_hidden_dims: List[int] = (64,)
    ...
    def __call__(self, x, x_critic):
        ...
        actor_outputs = self._apply_actor_hypernet(obs, agent_id)
        critic_outputs = self._critic_mlp(x_critic)

        if self.is_continuous:
            return (actor_outputs, self.log_std), jnp.squeeze(critic_outputs, axis=-1)
        return actor_outputs, jnp.squeeze(critic_outputs, axis=-1)

    def _apply_actor_hypernet(self, obs, agent_id):
        # One shared hypernetwork generates a distinct actor parameter set for
        # each agent embedding; the selected actor is then applied per sample.
        ...
        return actor_outputs_all[agent_id, batch_indices]
