...
class SafeGRUMAPPOTrainer:
    ...
    def __init__(self, config: Mapping[str, Any]):
        ...
        self.reward_critic_state = TrainState.create(
            apply_fn=self.reward_critic.apply, params=reward_params, tx=optimizer
        )
        self.cost_critic_state = TrainState.create(
            apply_fn=self.cost_critic.apply, params=cost_params, tx=optimizer
        )
        ...
        *,
        ...
        if algorithm == "macpo":
            ...
            def reward_objective(params):
                ...
                objective = jnp.mean((ratio - 1.0) * reward_advantages[:, :, None])
                if self.two_stage_intent:
                    residual = (
            ...
            def cost_objective(params):
                ...
                ratio = jnp.exp(new_log_probs - rollout.log_probs)
                return daily_cost + jnp.mean((ratio - 1.0) * cost_advantages[:, :, None])
        ...
        reward_loss, reward_grads = self._reward_critic_loss_and_grad(
            self.reward_critic_state.params,
            rollout.global_obs,
            rollout.dones_before,
        ...
        self.reward_critic_state = self.reward_critic_state.apply_gradients(grads=reward_grads)
        cost_loss, cost_grads = self._cost_critic_loss_and_grad(
            self.cost_critic_state.params,
            rollout.global_obs,
            rollout.dones_before,
        ...
        self.cost_critic_state = self.cost_critic_state.apply_gradients(grads=cost_grads)
        if algorithm == "lagrangian":
    ...
    def deterministic_rollout(
        ...
        *,
            ...
            for step in range(episode_length):
                ...
                if voltage_reference:
                    if self.action_dim < 6:
                        raise ValueError("voltage reference requires the Env-v6 H2 order action")
                    env_state = evaluation_env.envs[0].env
                    action = adjust_action(
                        action,
                        last_info,
                        soc=np.asarray(env_state.soc, dtype=np.float32),
                        pcc_power=np.asarray(
                            last_info.get("pcc_p_kw", np.zeros(self.num_agents)),
                            dtype=np.float32,
                        ),
                        bat_power=np.asarray(env_state.bat_power, dtype=np.float32),
                        el_cap=np.asarray(env_state.el_cap, dtype=np.float32),
                        h2_level=np.asarray(env_state.h2_level, dtype=np.float32),
                        pending_h2=np.asarray(
                            last_info.get("pending_h2_energy_agent", np.zeros(self.num_agents)),
                            dtype=np.float32,
                        ),
                    )
                ...
                # A reference rollout is intentionally a compact artifact:
                # ``actions`` are the actions actually sent to the environment
                # after coordination. Do not archive a second raw-action copy
                # or per-rule explanations; the summary and physical trace are
                # sufficient to compare the policy with the reference.
                if not voltage_reference:
                    step_record["intents"] = np.asarray(current_intents[0]).tolist()
        ...
        if self.two_stage_intent and records and not voltage_reference:
        ...
        return {
            "summary": {
                ...
                "voltage_reference": bool(voltage_reference),
                **({} if voltage_reference else intent_stats),
            },
            "steps": records,
        }
